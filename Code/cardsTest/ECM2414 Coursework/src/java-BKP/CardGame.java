...
public class CardGame extends Thread{
    ...
    public void setup(String stringNumberOfPlayers, String packFileName, Boolean testing) {
        Arrays.stream(Objects.requireNonNull(new File((GLOBALPATH + "ECM2414 Coursework\\doc\\output\\")).listFiles())).forEach(File::delete);
    ...
    public void dealCards(){
        dealCardsForPlayers();
        dealCardsForDecks();
        writeInitialHandOfPlayersToFile((GLOBALPATH + "ECM2414 Coursework\\doc\\output\\"));
    }
     ...
     * @param filepath the path to the output files
     */
    public void outputPlayerMsg(String sentence, int player, String filepath){
        try {
            FileWriter pWrite = new FileWriter((GLOBALPATH + filepath) + "player" + (player + 1) + "_output.txt", true);
        ...
        if(winner != - 1) {
            ...
            for(int i=0; i < numberOfPlayers; i++){
                if(winner == players.get(i).getPlayerID()){
                    outputPlayerMsg("\nPlayer " + (i + 1) + " wins", i, "ECM2414 Coursework\\doc\\output\\");
                } else{
                    outputPlayerMsg("\nPlayer " + (winner + 1) + " has informed player " + (i+1) + " that player " + (winner + 1) + " has won", i, "ECM2414 Coursework\\doc\\output\\");
                }
                outputPlayerMsg("\nPlayer " + (i + 1) + " exits", i, "ECM2414 Coursework\\doc\\output\\");
                outputPlayerMsg("\nPlayer " + (i + 1) + " final hand: " + players.get(i).getHandAsString(), i, "ECM2414 Coursework\\doc\\output\\");
                outputDeck("ECM2414 Coursework\\doc\\output\\");
            }
    ...
    public void outputDeck(String path){
        for(int i=1; i<=numberOfPlayers; i++) {
            try {
                BufferedWriter dWrite = new BufferedWriter(new FileWriter((GLOBALPATH + path) + "deck" + i + "_output.txt"));
    ...
    public synchronized void drawCard(int playerIndex) {
        ...
        outputPlayerMsg("\nPlayer " + (playerIndex + 1) + " draws a " + players.get(playerIndex).getHand().get(4) + " from deck " + (playerIndex + 1), playerIndex, "ECM2414 Coursework\\doc\\output\\");
    }
    ...
    public synchronized void discardCard(int playerIndex){
        ...
        outputPlayerMsg("\nPlayer " + (playerIndex + 1) + " discards a " + value + " to deck " + (discardDeckIndex + 1), playerIndex, "ECM2414 Coursework\\doc\\output\\");
        outputPlayerMsg("\nPlayer " + (playerIndex + 1) + "'s current hand is " + players.get(playerIndex).getHandAsString(), playerIndex, "ECM2414 Coursework\\doc\\output\\");
    }
